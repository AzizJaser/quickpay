...
|---|---|---|---|
| | **Biller down for five minutes** | The EOD sweep is **serial**: `fixedDelay` waits for completion and each `inquire` burns the full 2s timeout, so *N* stranded bills cost *N* × 2s per pass and the sweep stops keeping up with its own interval. | Resilience4j circuit breaker → **closes roadmap topic #5** |
